...
- If the latest user message states a current role that conflicts with Account/CV, prefer the user's latest message for background.role and the spoken summary.
...
- FOUND: set advance=true, fill background fields from evidence (prefer the user's stated role), and set response to ONE short personalized summary PLUS a natural career-direction question.
  Good example: "Nice — software developer for about 5 years at Paragon. Are you looking for a job now, aiming for a longer-term role in the future, or still figuring out what you want?"
  Bad example: repeating a CV title the user did not claim, or "You're now looking to [NEAR_TERM|DREAMJOB|GUIDED]."
